...
public class RsaUtils {
    ...
    public static byte[] decryptByPrivateKey(byte[] data, byte[] privateKey)throws NoSuchAlgorithmException, InvalidKeySpecException, NoSuchPaddingException, InvalidKeyException,BadPaddingException, IllegalBlockSizeException {
        ...
        byte[] realBytes = {};
        byte[] cache = {};
        while (cum < data.length) {
            if (cum + MAX_SIZE < data.length) {
                cache = cipher.doFinal(data, cum, MAX_SIZE);
                cum += MAX_SIZE;
            } else {
                cache = cipher.doFinal(data, cum, data.length-cum);
                cum = data.length;
            }
            realBytes = Arrays.copyOf(realBytes, realBytes.length+cache.length);
            System.arraycopy(cache, 0, realBytes, realBytes.length - cache.length, cache.length);
        }
        return realBytes;
    }
